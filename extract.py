...
if __name__ == "__main__":
    ...
    ap.add_argument("-a", "--hashes", required=True, type=str,
                    help="path to output hashes directory")
    ap.add_argument("-p", "--procs", type=int, default=-
                    1, help="# of processes to spin up")
    ...
    for(i, imagePaths) in enumerate(chunkedPaths):
        ...
        outputPath = os.path.sep.join(
            [args["output"], "proc{}.pickle".format(i)])
    ...
    # (i.e., will combine the results of each pickled/serialized)
    # dictionary into a *single* dictionary
